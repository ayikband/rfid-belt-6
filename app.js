...
function initHeader(){
  ...
  const alt = $('#altKaplama'); if (alt) alt.textContent = '2 mm';
}
initHeader();
openWelcomeModal();
...
// Örnek veri — PLATO 145°C
(function buildSampleCurve(){
  const pts = [];
  for(let t=0;t<=60;t+=2){
    let T;
    if(t<=10) {
      // 25°C → 145°C (lineer) ilk 10 dakikada
      T = 25 + (t/10) * (145 - 25);
    } else if(t<=40) {
      // 145°C plato (±2°C dalga)
      T = 145 + Math.sin((t-10)/30 * Math.PI) * 2;
    } else {
      // 145°C'den soğuma
      T = 145 - (t-40)*4;
    }
